...
class TournamentPredictor: public BranchPredictor
{
	...
	BOOL predict(ADDRINT addr)
        ...
        {
            // TODO: Update BHT according to branch results and prediction
	    m_BPs[0]->update(takenActually, m_BPs[0]->predict(addr), addr);
	    m_BPs[1]->update(takenActually, m_BPs[1]->predict(addr), addr);
	    if(m_BPs[0]->predict(addr) == takenActually && m_BPs[1]->predict(addr) != takenActually)
	        m_gshr->decrease();	  
	    else if(m_BPs[0]->predict(addr) != takenActually && m_BPs[1]->predict(addr) == takenActually)
		m_gshr->increase();
        }
};
