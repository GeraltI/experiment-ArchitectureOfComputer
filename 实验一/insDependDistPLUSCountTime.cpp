...
VOID updateInsDependDistance(VOID *v)
{
	...
	*/
	...
	for (vector<reg_t>::iterator it = regs->write.begin(); it != regs->write.end(); it++)
		// 对任意的r属于regs->write, 将当前PC值赋值给lastInsPointer[r]
	...
	for (vector<reg_t>::iterator it = regs->read.begin(); it != regs->read.end(); it++)
	{
		...
		{
			// 对任意的r属于regs->read, 其依赖距离 = 当前PC值 - lastInsPointer[r]
...
VOID Trace(TRACE trace, VOID *v)
{
	...
	usedTime = usedTime + (endTime.tv_sec*1000000 + endTime.tv_usec) - (startTime.tv_sec*1000000 + startTime.tv_usec);
}
